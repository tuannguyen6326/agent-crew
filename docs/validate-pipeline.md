...
`ac-ship.sh review-agent` is a thin adapter to `ac-verify codereview`. The facade leases an isolated exact-ref worktree, launches one fresh `ac-pane-agent` round, validates the structured verdict, captures evidence, then reaps the pane and returns the lease. A verdict that fails an envelope check (no JSON object, off-shape findings, a foreign `reviewed_ref`, an off-type risk field) gets exactly one correction turn: a fresh pane is handed the rejected payload and the failed check, fixes the wrapping and no finding content, and its corrected object becomes that round's verdict at the same `reviewed_ref` (`.correction.failed_check` on the result records it); a content failure such as an undispositioned prior id gets no turn. A rejected verdict (the pane completed and its round evidence is durable, only the output failed the schema, twice when a correction ran) releases those same runtime resources rather than orphaning them; a run that produced no usable result before any verdict existed (pane crash, missing transcript, a non-ok terminal status) is reaped the same way. Only an incomplete QA export - reconciliation or evidence-export failing after a valid pane verdict - still preserves its pane/lease/meta for recovery.
