...
There is no resumable reviewer session, headless fallback, or separate staged code-review reviewer. A ref-changing fix launches one fresh sequential round over the current full diff; only structured finding history carries forward, never old PASS/context authority. One `(family, verifier-kind)` has at most one live pane.
