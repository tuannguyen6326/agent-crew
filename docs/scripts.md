...
| `ac-standing-jobs.sh [--ids]` | Session-start ride-along: each DECLARED standing job's state from `records/standing-jobs.md`. `--ids` prints just the declared ids, the one parser of that field, for `ac-rig.sh`. Spec: `bin/ac-standing-jobs.sh` header. |
| `ac-rig.sh drift` | Compare the rig manifest `records/rig.json` against reality and report every divergence: the home binding, the distro checkout `state/.ac-root` names, the config surface (declared knobs and both-direction closure), and the standing-job id set. Three verdicts - `OK`/`DRIFT`/`UNVERIFIABLE`; exit 1 on any drift, 2 on an unreadable manifest. Spec: `bin/ac-rig.sh` header. |
...
| `ac-epic-branch.sh create\|verify\|show\|retire` | The per-epic INTEGRATION-BRANCH verbs (epic-branch-mech): `data/<epic>/branches` records `<repo> <branch> [push=yes staging=<b>]` per repo, archive-aware (`ac_epic_branches_file`/`ac_epic_base_for`, ac-lib.sh); `create` cuts at the shared freshest-tip resolver and never moves an existing branch; `verify` is quiet enough to gate on; `retire` is the deliberate end of the fence. The fence itself lives in `ac-tree.sh get` (leases cut from the recorded branch, fail-closed when missing), landings in `ac-merge-local.sh` (ref-only ff, or in-place ff on a checked-out target), and the per-epic review derivation in `ac-brief.sh` (AGENTS.md section 5 epic exception). Spec: `bin/ac-epic-branch.sh` header. |
| `ac-domain.sh new\|assign\|unassign\|queue\|retire\|list\|validate` | The CREWDOMAIN verb layer (crewdomain-token): build the three-member package at `crewdomains/<name>/`, stamp/strip the `domain:<name>` token on fleet `## Queued` rows IN PLACE, print a domain's slice of the one ledger (`queue`), retire fail-closed, render the digest block (ORPHAN-TOKEN census included), and check the registry, the project view and token discipline. Separate from `ac-deputy.sh` in every way - own registry, own root, own verbs. Spec: `bin/ac-domain.sh` header. |
...
| `ac-ledger-guard.sh` | PreToolUse hook refusing a scoped session's write to `records/backlog.md`, `records/projects.md` or `records/captain.md`. Spec: `bin/ac-ledger-guard.sh` header. |
