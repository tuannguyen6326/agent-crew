...
description: Crewchief fleet-level project lifecycle judgment - add, clone, create, initialize, change the default mode of, retire, or remove a project registered in records/projects.md. Load before any such lifecycle action. Removal is destructive and always requires explicit captain confirmation after the exact target and its unlanded-work preflight are resolved. Does not own per-task delivery-mode triage (AGENTS.md section 4) or CrewDeputy home clones (CrewDeputy provisioning and convergence own them).
---

# project-management

Judgment for the fleet-level lifecycle of a registered project: add, clone, create, initialize, change the standing mode of, retire, or remove it.
...
Load before adding, cloning, creating, initializing, changing the default mode of, retiring, or removing a registered project.

Out of scope - keep these with their owners:

- per-task delivery-mode triage (which mode THIS task ships in) stays in `AGENTS.md` section 4 and the intake ladder;
...
Before adding a project, resolve: the source or creation intent; the unused local name and destination; the default delivery mode; the `+yolo` posture only when explicitly requested; and whether the selected mode requires an origin remote.
...
- Before remote creation, the captain must approve the repository name, the owner or organization, the visibility, and the default delivery mode.
...
- A per-task delivery-mode override never changes the project registry default. Change the registry default only when the captain changes the project's standing posture.
...
- `/debrief` may RECORD an already completed project lifecycle change, but never performs project creation, removal, or mode mutation as cleanup.
