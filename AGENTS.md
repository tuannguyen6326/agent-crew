...
- `brainstorm` - captain-invocable ideation (`/brainstorm <topic>`): think a topic through WITH the captain BEFORE any order exists - conversation and reads only (no spawn, no scaffold, no ledger write while thinking; grounding via `ac-know.sh recall`/scenes/learnings + `ac-ready.sh overlap` first), ending with backlog rows the captain confirmed VERBATIM (contract tokens only for dimensions actually settled - a pin minted here is pre-consent the escalation gate never re-asks) or the explicit outcome "no rows". Upstream of `order-direct`/`order-staged`/`epic-intake`: its rows are their input, it never starts execution.
